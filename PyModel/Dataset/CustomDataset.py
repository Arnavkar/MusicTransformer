...
class BaseDataset():
    ...
    def get_maestroJSON(self, path="./data/raw/maestro-v3.0.0/maestro-v3.0.0.json"):
        with open(path) as f:
            data = json.load(f)
        return data
        ...
        if min_duration is not None:
            ...
            while i < len(midi_filenames_from_json):
                ...
                if duration < min_duration:
                    self.logger.info(f"Removed file {midi_filenames_from_json[i]}, Duration of is {duration}, which is less than {min_duration}")
                    del midi_filenames_from_json[i]
                i+=1
            self.logger.info(f"Number of midi files remaining after removing files less than {min_duration} seconds: {len(midi_filenames_from_json)}")
        ...
        if min_event_length is not None:
            ...
            while i < len(midi_filenames_from_json):
                ...
                if len(data) < min_event_length:
                    self.logger.info(f"Removed file {midi_filenames_from_json[i]}, Length of is {len(data)}, which is less than {min_event_length}")
                    del midi_filenames_from_json[i]
                i+=1
            self.logger.info(f"Number of midi files remaining after removing files less than {min_event_length} events: {len(midi_filenames_from_json)}")
...
class CustomDataset(BaseDataset,tf.keras.utils.Sequence):
    def __init__(self, p:Params, mode, path="./data/processed",
                min_duration=None,
                min_event_length=None,
                num_files_to_use=None,
                logger = None):
        super().__init__(p, mode, path, min_duration, min_event_length, num_files_to_use, logger)
    ...
    def on_epoch_end(self):
        self.logger.info("Epoch ended, resetting fileData objects")
